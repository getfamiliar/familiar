...
export type ScheduledSubagentOp = "u" | "d";

/** Parsed notification handed to {@link ScheduledSubagentBus.listen} subscribers. */
export interface ScheduledSubagentNotification {
    readonly key: string;
    readonly op: ScheduledSubagentOp;
}
...
/**
 * Domain client for the `scheduled_subagents` table — agent-requested
 * one-off wake-ups. Used by both sides of the bus:
 *
 * - Container: the `schedule_subagent` / `unschedule_subagent` /
 *   `get_scheduled_subagents` tools call {@link upsert} / {@link deleteByKey}
 *   / {@link listInRange}.
 * - Host: the `ScheduledSubagentScheduler` calls {@link listFuture} at
 ...
 */
export class ScheduledSubagentBus {
    ...
    /**
     * Insert a scheduled subagent or replace the existing row with the
    ...
    async upsert(row: NewScheduledSubagent): Promise<ScheduledSubagentRow> {
        const result = await this.connection.getPool().query<RawRow>(
            `INSERT INTO scheduled_subagents
    ...
    async deleteByKey(key: string): Promise<boolean> {
        const result = await this.connection
            .getPool()
            .query(`DELETE FROM scheduled_subagents WHERE key = $1`, [key]);
        return (result.rowCount ?? 0) > 0;
    }

    /** Fetch a row by key, or `undefined` when missing. */
    async getByKey(key: string): Promise<ScheduledSubagentRow | undefined> {
        const result = await this.connection
            .getPool()
            .query<RawRow>(`SELECT * FROM scheduled_subagents WHERE key = $1`, [key]);
     ...
     */
    async claimAndDeleteForFiring(key: string): Promise<ScheduledSubagentRow | undefined> {
        const result = await this.connection
            .getPool()
            .query<RawRow>(`DELETE FROM scheduled_subagents WHERE key = $1 RETURNING *`, [key]);
        return result.rows.length > 0 ? mapRow(result.rows[0]) : undefined;
    }
     ...
     */
    async listInRange(fromUtc: string, toUtc: string): Promise<ScheduledSubagentRow[]> {
        const result = await this.connection.getPool().query<RawRow>(
            `SELECT * FROM scheduled_subagents
     ...
     * for every still-future scheduled subagent.
     */
    async listFuture(nowUtc: string): Promise<ScheduledSubagentRow[]> {
        const result = await this.connection.getPool().query<RawRow>(
            `SELECT * FROM scheduled_subagents
             WHERE fire_at >= $1
             ORDER BY fire_at ASC`,
    ...
    async deletePastDue(nowUtc: string): Promise<number> {
        const result = await this.connection
            .getPool()
            .query(`DELETE FROM scheduled_subagents WHERE fire_at < $1`, [nowUtc]);
        return result.rowCount ?? 0;
    }

    /**
     * Subscribe to {@link SCHEDULED_SUBAGENTS_CHANNEL}. Parses the
     ...
     */
    async listen(
        handler: (notification: ScheduledSubagentNotification) => void | Promise<void>,
    ): Promise<ScheduledSubagentUnsubscribe> {
        const wrapper: NotificationHandler = (payload) => {
            this.log?.debug(
                { channel: SCHEDULED_SUBAGENTS_CHANNEL, payload },
                "NOTIFY scheduled_subagents_changed",
            );
            void this.dispatch(payload, handler);
        };
        await this.connection.listen(SCHEDULED_SUBAGENTS_CHANNEL, wrapper);
        ...
        return async () => {
            ...
            disposed = true;
            await this.connection.unlisten(SCHEDULED_SUBAGENTS_CHANNEL, wrapper);
        };
    }
    ...
    private async dispatch(
        ...
        if (colon < 0) {
            this.log?.warn(
                { payload },
                "ScheduledSubagentBus: malformed NOTIFY payload (no colon)",
            );
            return;
        }
        ...
        if (op !== "u" && op !== "d") {
            this.log?.warn({ payload, op }, "ScheduledSubagentBus: unknown op in NOTIFY payload");
            return;
        }
        try {
            ...
            this.log?.error(
                ...
                "ScheduledSubagentBus listen handler error",
            );
        }
    }
}
...
function mapRow(raw: RawRow): ScheduledSubagentRow {
    return {
        key: raw.key,
