 ...
 * report in an in-memory registry that the `tools list` CLI reads. This
 * is the single source of truth — there is no hand-maintained host-side
...
export interface ContainerToolInfo {
    ...
    /**
     * Security classification (mirrors `CONTAINER_TOOL_LEVELS` in the
     * container). See {@link ToolLevel}; `default` unless the built-in
     * is listed there.
     */
