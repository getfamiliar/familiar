...
- `schedule_handler({handler, topic?, prompt?, payload?, when?, key?})` — fire-and-forget. Without `when`, the new agentrun runs as a child of the current run under the same event; the caller does not see its result. With a future `when` (ISO-8601 wall-clock in the user's local timezone), the host defers it and fires a fresh event at that time; pass a stable `key` to make rescheduling idempotent (re-using the key overwrites the previous schedule), or omit it for an auto-generated id. A `when` that parses to a past instant silently falls back to the immediate path. Companion tools: `unschedule_handler({key})` cancels a pending schedule, `get_scheduled_handlers` lists them.
...
`schedule_handler` (immediate mode) and `call_handler` children inherit `event_id` and `privileged` from the parent; `topic` defaults to the parent's when omitted. This produces an arbitrary, branching workflow tree rooted at `<topic>/index.md` instead of a fixed pipeline. **The root agentrun is the orchestrator of its event**: the event's terminal state is the root's terminal state. Immediate `schedule_handler` children are out-of-band side effects (they will not flip the event's outcome on their own — useful for fan-out fire-and-forget); `call_handler` children fold their result back into the parent, so they participate in the root's outcome through the chain of returns. Deferred `schedule_handler` wake-ups always run under a fresh event (no parent). The same suspend/resume mechanism will back the approval gate (see open question #1).
...
4. **`AgentrunScheduler`** picks the row (`pending → running`), resolves the handler markdown file (see "Handler resolution"), and runs an agent session against it. The handler's content is the prompt; the agent has MCP tools plus the `schedule_handler` / `call_handler` subagent tools.
5. **Inside the handler**, the agent decides what to do next. It may call MCP tools, edit workspace files, and spawn subagents via `schedule_handler` (fire-and-forget when called without `when`) or `call_handler` (suspend-and-await) — e.g. `index.md` triages, fans `respond` off as a queued child, and calls `analyze` to react to its summary. Each child is another `agentruns` row with `parent_agentrun_id` set to the spawning row, `event_id` propagated, and `calltype` set to `'queued'` or `'called'`. A `schedule_handler` call with a future `when` produces no child agentrun under the current event — it inserts a `scheduled_handlers` row instead, and the host fires it later as a fresh event.
...
The override applies uniformly to all handlers, not just `index`. Validation happens at subagent-spawn time: both `schedule_handler` and `call_handler` resolve the path and refuse if neither file exists, so the calling agent gets a synchronous error.
...
3. **Handler resolution + subagent tools** — workspace path resolver with sub-topic override fallback, plus the in-handler `schedule_handler` (fire-and-forget when called without `when`, deferred fresh-event when called with one) and `call_handler` (suspending) tools that spawn child agentruns by basename and validate at spawn time.
